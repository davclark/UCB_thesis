\documentclass[12pt]{myucthesis}
% This enables, e.g., clicking in Skim's PDF view (on OSX) to open the relevant
% source file in Vim
\synctex=1

% \nofiles
% The above command prevents latex from writing its auxiliary
% files. This is useful if you want to manually tweak them before you
% generate your final PDF. But it will break reference / bibliography
% resolution!

% XeTeX font setup. The microtype stuff gives fancy typography
% improvements that make things look (very) subtly better.
% DJC - I'm doing all kinds of fussing around to get decent math type setting

% This lets us load OpenType fonts from the system without any fuss
\usepackage{fontspec}
% \usepackage{aasmacros,amsmath,amssymb}
% Note that if you use this, you need to do it after amsmath (and prehaps other
% ams things) Using unicode-math as recommended by the libertine package
% documentation
% mathspec would also need to be defined after amsmath
% This is useful if you decide to use Libertine
\usepackage[libertine]{newtxmath}
% I have no idea what this actually does, but it's related
% \usepackage{newtxtext}

% This is one approach to doing maths in the context of libertine
% \usepackage[partial=upright,nabla=upright]{unicode-math}
% \usepackage{unicode-math}

% I am fairly certain this is a complete waste of time...
\newfontfeature{Microtype}{protrusion=default;expansion=default;}
% Times New Roman is installed by default, but Linux Libertine is classier and
% Open Source

% Handled more straightforwardly (and without extra installs!) just using the
% libertine argument in TeX Live

% \setmainfont[Microtype,Ligatures=TeX]{TeX Gyre Pagella}
\setmainfont[Microtype,Ligatures=TeX]{Linux Libertine O}
% \setmainfont[Microtype,Ligatures=TeX]{Times New Roman}

% \setmonofont{TeX Gyre Heros}
\setmonofont[Microtype]{Linux Libertine Mono O}

% Note - there is no corresponding _non_ math font for Asana (i.e., no bold...)
% \setmathfont{Asana Math}
% \setmathfont[Microtype]{TeX Gyre Pagella Math}
% \setmathfont[range={\mathcal,\mathbfcal},StylisticSet=1]{XITS Math}
% \setmathfont[Scale=MatchLowercase]{Neo Euler} 
% \setmathfont[range=\mathup]{Linux Libertine O}
% \setmathfont[range=\mathit]{Linux Libertine O Italic}
% \setmathfont[range=\mathbfup]{Linux Libertine O Bold}
% \setmathfont[range=\mathbfit]{Linux Libertine O Bold Italic}

% Page layout. The fancyhdr package may complain about the need for a
% larger headheight, depending on how long chapter titles are; if left
% unspecified in the geometry setup, it defaults to 12pt. The
% "showframe" option causes the geometry package (version >= 5.0) to
% show a frame around the margins on every page, which is great for
% checking that you don't overflow anywhere.

%\usepackage[xetex,letterpaper,includehead,margin=1in,headheight=15pt,showframe]{geometry}
\usepackage[xetex,letterpaper,includehead,margin=1in,headheight=15pt]{geometry}
\usepackage{fancyhdr}
\pagestyle{fancyplain}
\lhead[\fancyplain{\thepage}{\thepage}]{\fancyplain{}{\scshape\rightmark}}
\rhead[\fancyplain{}{\scshape\leftmark}]{\fancyplain{\thepage}{\thepage}}
\chead{}
\cfoot{}
\lfoot{}
\rfoot{}


% Bibliography stuff:
% DJC - I'm now using biblatex and bilatex-apa
% This is 1i18n overkill, including i18n of quotes
\usepackage[american]{babel}
\usepackage{csquotes}

\usepackage[style=apa]{biblatex}

% More i18n overkill
\DeclareLanguageMapping{american}{american-apa}

\addbibresource{thesis.bib}
\addbibresource{fixed.bib}

% Other setup:

% Note: with BibLaTeX, you should load hyperref after the biblatex package.
\usepackage[pdfborder={0 0 0},pdfusetitle]{hyperref}
% This was the original hyperref inclusion with color
% \usepackage[colorlinks,urlcolor=blue,citecolor=blue,linkcolor=blue,pdfusetitle]{hyperref}
\usepackage{pdflscape} % allows landscape-oriented figures with PDF page rotation
\usepackage{graphicx}

\usepackage{mydeluxetable} % deluxetable customized to play well with ucthesis

% DJC - added subfig, multirow, hologo here - the custom 'mydeluxtable' above is
% included in the template, check if you get the chance!
\usepackage{subfig,multirow,hologo}

\begin{document}
\ssp % single spacing
\hypersetup{pageanchor=false}
\include{setup}
\maketitle
\copyrightpage

\begin{abstract}
My work is awesome. Give me a Ph.D.
\end{abstract}

% DJC: another hyperref config
\hypersetup{pageanchor=true}
\begin{frontmatter}

\begin{dedication}
\null\vfil
{\large
\begin{center}
For my grandma. And other people who also helped.
% Shout out to Larry Goldfarb.
\end{center}}
\null\vfil
\end{dedication}

\tableofcontents
\listoffigures % optional
\listoftables % optional

% If using code.sty, can also add:
%% \listofcodes
%% \addcontentsline{toc}{chapter}{List of Code Examples}

\begin{acknowledgements}
Thanks for all the help!

% Feel free to modify or remove this acknowledgment:
This dissertation was based on the
\href{https://github.com/pkgw/ucastrothesis}{\textsf{ucastrothesis}}
% DJC non-hyperref alternative with plain textsf
% \textsf{ucastrothesis}
template, processed with \hologo{XeLaTeX} using the Linux Libertine font.

\end{acknowledgements}
\end{frontmatter}

\include{intro/intro}
\include{mechanism/mechanism}
\include{two-routes-estimation/two-routes-estimation}
\include{pro-ndi/pro-ndi}
\include{evil-ndi/evil-ndi}

% These should get re-factored into proper chapters
\include{experimental-description/experimental-aims}
\include{experimental-description/experimental-plan}

\include{conclusion/conclusion}

% \bibliography{proposal,fixed}{}
% This is how biblatex does it
\printbibliography[heading=bibintoc]

\appendix

\include{appendices/kappa-extension/kappa-extension}
\include{appendices/graphical-models/graphical-models}
\include{appendices/400words/400words}
\include{appendices/survey-items/survey-items}
\include{appendices/numbers/numbers}

\end{document}
