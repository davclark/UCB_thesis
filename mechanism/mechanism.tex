...
anthropogenic (i.e., “human caused”) climate change. Informally, Michael Ranney,
...
myself and Ronald Cohen) proceeded to develop a short \~400-word description of
the mechanism. This text is reproduced in full in Appendix~\ref{app:400words}.
...
describing \emph{your} understanding either aloud or on paper.\footnote{If you
    ...
    modify the exercise to describing the mechanism of the greenhouse effect,
    first described by Nobel laureate Svante Arrhenius in
    \citeyear{arrhenius_influence_1896}.}
...
As described in Chapter~\ref{chap:survey}, pretest responses can be used to
...
respect. Below, we report data from the 85 Berkeley and 41 Brownsville students
who completed the survey as intended and had been U.S. residents for ten years
or more (because we expressly consider U.S. exceptionalism/nationalism). Of the
Berkeley data, we analyzed 43 no-pretest surveys and the pretest part of 42
sandwich surveys---but due to anticipated time constraints, only 30 sandwich
posttests could be completed/obtained. Of the Brownsville data, we analyzed 22
no-pretest and 19 complete sandwich surveys.
...
full in Appendix~\ref{app:kappa}. This reliability was high (weighted $κ = .71$
based on about one-third of the Berkeley data; $κ = .67$ across the full
...
Significant improvements ($p < .05$ for all 6 improvement possibilities across
...
mention the mechanism in pretest (11/42), but they did on the posttest (26/30
...
\begin{figure}
    ...
    \caption{Self-rated knowledge scores for participants in our classroom
        ...
        < .01$), while improvements for our no-pretest groups were non-existent
...
acceptance on the posttests, as compared to pretest measures ($t(72) = 2.28$,
...
\begin{figure}
    ...
    \caption{Changes in mean of Climate Change related beliefs and attitudes.
        ...
        imputation ($p < .01$), as well as looking only at the sandwich group
        ($p < .01$).}
...
students’ naïve pretest data, in which participants’ self-perceived ratings of
...
Brownsville students ($r = .15$, $p = .55$). This may be reflective of overall
...
(11/42 responses recieved a mechanistic code), but they often do on the
...
posttest, which has a lesser prevalence of mechistic responses on the posttest
of the two conditions &	3.2E-07	&
Fisher's exact test (two-sided)	\\
Misconceptions are common in the pretest but not the post test, total .38 pre
to .10/.12 posttest (sandwich \& no-pretest groups). Ozone .19 to .03/.02 (S/N),  wrong
GHG  .24 to .07/.09 (S/N). (test on total misconceptions, comparing sandwich
...
Use of infrared is greater posttest than pretest. Goes from 0 to 16 / 22 in S
...
&	$t(79.02) = -4.06$ (Welch)	\\
Sandwich: Energy Objecive knowledge scores improve after the blurb	&	.04
&	$t(29) = -2.15$ (paired)	\\
No pretest: Energy Objecive knowledge scores improve after the blurb	&
4.60E-04	&	$t(80.82) = -3.6547$ (Welch)	\\
Differences in GW attitudes are significant	&	.013	&	$t(72) = -2.28$
(paired / imputed)	\\
...
Surprise is significantly greater in S group than N group	&	.053	&
$t(42.08)=1.65$ (Welch)	\\
...
group's posttest answers for know1	&	2.5E-4	&	$t(82.91) = -383$
(paired)	\\
...
inconvenient truth and warming attitudes	&	.022	&	$r(41) = .309$	\\
...
and GW attitudes on the pretest ONLY (differences in self-rated knowledge are
also insigificant). NB: we predicted the opposite result!	&	.012	&
$r(40) = .386$	\\
\emph{Post hoc:} Sandwich: Negative correlation between posttest self-rated
knowledge and CHANGE in objective score	&	.011	&	$r(28) = -.458$	\\
...
Carefulness is significantly correlated with posttest GW attitudes.	&	.035
&	$r(41) = .279$	\\
...
Sandwich: GHG Objecive knowledge scores improve after the blurb	&	.0034	&
$t(18) = 3.38$ (paired)	\\
No pretest: GHG Objecive knowledge scores improve after the blurb	&
5.8E-4	&	$t(33.2) = 3.81$ (Welch)	\\
Sandwich: Light Objecive knowledge scores improve after the blurb	&	.0095
&	$t(18) = 2.9$ (paired)	\\
No pretest: Light Objecive knowledge scores improve after the blurb	&	1.4E-4
&	$t(25.6) = 4.48$ (Welch)	\\
Sandwich: Energy Objecive knowledge scores improve after the blurb	&	.02
&	$t(18) = 2.5$ (paired)	\\
No pretest: Energy Objecive knowledge scores improve after the blurb	&
...
No pretest: posttest increase in self rated knowledge (compared to S pretest) is
...
pretest	&	.55	&	$r(17) = .15$	\\
Surprise is not significantly greater in S group than N group	&	.18	&
$t(38.1) = 0.92$ (Welch)	\\
...
no-pretest groups in terms of posttest attitudes (with differences going in
opposite directions across our two groups). Thus, it seems unlikely that a
pretest incurs a greater burden of experimental demand over the core
intervention (400 words followed by a posttest). Moreover, in some populations,
the pretest may help to anchor self assessment (as with our UT data). And
...
posttest attitude surveys to add reliability to the related RTMD metrics
...
RPP recruitment allowed us to administer a pretest to about half of the students
($n=36$, one was removed due to technical problems) between 8 and 26 days
($\mu=18.5$ days) before any of the 80 participated in the study, which may have
allayed test-retest effects (although we found little evidence for them in the
experiments reported above).  Thus, as with the above, some participants
received the full survey testing ``sandwich'' while others lacked the
demographics and attitude portion of the pretest. Note that this differs from
...
have them for all participants. Thus, for some participants, we collected some
demographic information twice (some of which is reported on below).
...
($\mu=4$ days). This delayed test had the same format as the immediate posttest.
This range of delays prior to this test was used to assess the
...
based on data from the RPP prescreening phase (completed 324 times, though some
...
(this was one one of the participants who took the RPP pretest).
...
% TODO: Grab conservativism values here!
...
.84$; economic conservativism was correlated at .68). The addition of economic
...
from 3.8 on pretest to 6.5 posttest and 6.3 on delayed test (gains from pretest
were significant at $p<.0001$ for both subsequent scores). These are plotted in
...
\begin{figure}
    ...
    \caption{Scored knowledge before and after our intervention. Faint lines
        ...
        robust, smooth regression line. Note that the pretest here was
...
Self-rated knowledge means also increased markedly from pre to posttest (4.5
...
\begin{figure}
    ...
    \caption{Self-rated knowledge before and after our intervention. Note that the
        pretest here was immediately prior to instruction.}
...
consensus) increased from a 6.20 pretest mean to a 6.54 posttest mean
($t(79)=2.5$, $p=.006$; a healthy improvement on our 1--9 Likert scales!). Some
of this improvement diminished over the following days, but most was retained:
the mean score on the delayed posttest was 6.44 ($t(79)=1.7$, $p=.05$). 
...
improvements even from pre to posttest. Upon closer inspection, the breakdown
for GW attitudes is quite different across sandwich and no-pretest groups, as
seen in Table~\ref{table:RPP-mech-gw-breakdown}. Unlike in previous studies, the
sandwich group contained a good number of participants who were recruited for
their conservativism (and there is a clear group difference on this measure).
...
Individual scores are depicted in Figure~\ref{fig:RPP-mech-GW}. While we may be
able to salvage this data using more sensitive techniques, such efforts are
unwarranted given the robust successes reported in
Sections~\ref{sec:mech-classroom} and \ref{sec:mech-mturk}.
...
\caption{Mean GW ratings for sandwich and no-pretest groups. Note that the
    ...
    The no-pretest group, however, starts much higher and stays higher.}
\label{table:RPP-mech-gw-breakdown}
\begin{tabular}{rccc}
  \toprule
 & Pretest & Posttest & Delayed test \\ 
  \midrule
  No (RPP) pretest & NA & 6.68 & 6.65 \\ 
...
\begin{figure}
    ...
    \caption{Reported global warming (GW) beliefs and attitudes before and after
        our intervention. Note that the pretest here was administered on
        average 18.5 days prior to instruction as a part of UC Berkeley's
        undergraduate participant pool (RPP) prescreening. The fact that over
        half of our participants are missing pretest data pulls the initial
        value of our regressor up markedly from the true pretest mean of 6.2.
...
\begin{figure}
    ...
    \caption{Surprise ratings for individuals in Study 2. Note that there is
        ...
        knowledge and attitude pretest or not. Moreover, these distributions
...
marginally significant (and smaller) correlation between na\"ive pretest
self-rated knowledge and GW attitudes ($r(34)=.27$, $p=.11$). Thus, while the
...
Delayed followup tests occurred over a range of
...
failed to exceed our significance threshold (4.5, $p=.21$). These scores are
